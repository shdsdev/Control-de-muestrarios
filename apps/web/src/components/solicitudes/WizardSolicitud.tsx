...
export default function WizardSolicitud({ isOpen, onClose, onSuccess, defaultEmpresaId }: WizardSolicitudProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-3xl p-4 md:p-8 animate-in fade-in duration-500">
            <div className="w-full max-w-5xl h-full max-h-[90vh] bg-zinc-950/50 border border-white/10 rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl relative">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white tracking-tight uppercase">
                                Nueva Solicitud
                                {formData.tipo_solicitud === 'fabricacion' && <span className="text-primary ml-2">· Fabricación</span>}
                    ...
                    {Array.from({ length: 9 }).map((_, i) => (
                        <div
                            ...
                            className={cn(
                                ...
                                currentStep > i + 1 ? "bg-primary" : currentStep === i + 1 ? "bg-primary shadow-[0_0_15px_rgba(212,255,112,0.4)]" : "bg-white/5"
                            )}
                        />
                    ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                    {currentStep === 1 && (
                        <div className="max-w-xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">¿Quién solicita?</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Confirma el usuario titular de este pedido</p>
                            </div>

                            <div className="grid gap-4">
                                <div className="p-6 rounded-3xl bg-primary/5 border border-primary/20 flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/10 flex items-center justify-center shrink-0">
                                        <User className="text-primary" size={28} />
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-sm font-bold text-white">{userProfile?.nombre_completo || 'Usuario'}</p>
                                        <p className="text-[10px] text-primary font-black uppercase tracking-widest mt-1">Sesión Activa · {userProfile?.email}</p>
                                    </div>
                    ...
                    {currentStep === 2 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-[600px] min-h-0">
                            <div className="text-center space-y-2 shrink-0">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Selección de Empresa</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">¿Para qué marca es este producto?</p>
                            </div>

                            {/* Sticky Search Bar */}
                            <div className="sticky top-0 z-10 bg-zinc-950/80 backdrop-blur-xl p-4 rounded-3xl border border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between max-w-4xl mx-auto w-full shadow-2xl shrink-0">
                                <div className="relative flex-1 w-full">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-xs focus:ring-1 focus:ring-primary focus:outline-none transition-all text-white placeholder:text-zinc-600 shadow-inner"
                                    />
                                </div>
                                <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 shrink-0">
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            empresaViewMode === 'grid' ? "bg-primary text-black shadow-lg" : "text-zinc-500 hover:text-white"
                                        )}
                                    >Grid</button>
                                    <button
                                        ...
                                        className={cn(
                                            "px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
                                            empresaViewMode === 'list' ? "bg-primary text-black shadow-lg" : "text-zinc-500 hover:text-white"
                                        )}
                                    >Lista</button>
                                </div>
                            </div>
                                            ...
                                            {empresas
                                                ...
                                                .map(empresa => (
                                                    <div
                                                        ...
                                                        className={cn(
                                                            "group p-4 rounded-3xl border transition-all duration-300 cursor-pointer flex flex-col items-center gap-4 hover:shadow-[0_0_30px_rgba(212,255,112,0.05)]",
                                                            formData.empresa_id === empresa.id ? "bg-primary/10 border-primary shadow-[0_0_20px_rgba(212,255,112,0.1)]" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                                        )}
                                                    >
                                                        <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center overflow-hidden shrink-0 shadow-lg group-hover:scale-105 transition-transform">
                                                        ...
                                                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest text-center group-hover:text-white transition-colors">{empresa.nombre}</span>
                                                    </div>
                                                ))}
                                        </div>

                                        {subEmpresas.length > 0 && (
                                            <div className="pt-8 border-t border-white/5 space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                                                <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em] text-center italic">Sub-marcas vinculadas</h4>
                                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                                    {subEmpresas
                                                        ...
                                                        .map(sub => (
                                                            <div
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    formData.empresa_id === sub.id ? "bg-violet-500/10 border-violet-500 shadow-[0_0_20px_rgba(168,85,247,0.1)]" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                                                )}
                                                            >
                                                                <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center overflow-hidden shrink-0 group-hover:scale-105 transition-transform">
                                                                    {(empresaSearch && sub.logo_url && !brokenLogos.has(sub.logo_url)) ? (
                                                                        ...
                                                                        <Building2 className="text-zinc-800" size={20} />
                                                                    )}
                                                                </div>
                                                                <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest text-center group-hover:text-white transition-colors">{sub.nombre}</span>
                                                            </div>
                                                        ))}
                                                </div>
                                            </div>
                    ...
                    {currentStep === 3 && (
                        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Tipo de Solicitud</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Selecciona el flujo de trabajo</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        formData.tipo_solicitud === 'fabricacion' ? "bg-primary/10 border-primary" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                    )}
                                >
                                    <div className={cn(
                                        "w-16 h-16 rounded-3xl flex items-center justify-center shrink-0 transition-all duration-500",
                                        formData.tipo_solicitud === 'fabricacion' ? "bg-primary text-black" : "bg-zinc-900 text-zinc-500 group-hover:text-zinc-300"
                                    )}>
                                        <Package size={32} />
                                    </div>
                                    <div className="space-y-3">
                                        <h4 className="text-xl font-bold text-white uppercase tracking-tight">Fabricación</h4>
                                        <p className="text-xs text-muted-foreground uppercase tracking-widest leading-relaxed">Pedido de modelos ya existentes en el inventario de la marca.</p>
                                    </div>
                                    {formData.tipo_solicitud === 'fabricacion' && <div className="absolute top-4 right-4 text-primary"><CheckCircle2 size={24} /></div>}
                                </div>
                                ...
                                <div
                                    ...
                                    className={cn(
                                        "group p-8 rounded-[2rem] border transition-all duration-500 cursor-pointer relative overflow-hidden flex flex-col gap-6",
                                        formData.tipo_solicitud === 'diseno' ? "bg-primary/10 border-primary" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                    )}
                                >
                                    <div className={cn(
                                        "w-16 h-16 rounded-3xl flex items-center justify-center shrink-0 transition-all duration-500",
                                        formData.tipo_solicitud === 'diseno' ? "bg-primary text-black" : "bg-zinc-900 text-zinc-500 group-hover:text-zinc-300"
                                    )}>
                                        <Palette size={32} />
                                    </div>
                                    <div className="space-y-3">
                                        <h4 className="text-xl font-bold text-white uppercase tracking-tight">Diseño de Propuesta</h4>
                                        <p className="text-xs text-muted-foreground uppercase tracking-widest leading-relaxed">Creación de un nuevo modelo con branding y colores personalizados.</p>
                                    </div>
                                    {formData.tipo_solicitud === 'diseno' && <div className="absolute top-4 right-4 text-primary"><CheckCircle2 size={24} /></div>}
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {currentStep >= 4 && formData.tipo_solicitud === 'fabricacion' && (() => {
                        ...
                        return (
                            <div className="flex items-center justify-center gap-2 flex-wrap py-3 px-4 rounded-2xl bg-white/[0.02] border border-white/5 animate-in fade-in duration-300">
                                {selectedEmpresa && (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-white/10 text-[10px] font-bold text-zinc-300 uppercase tracking-widest">
                                        <Building2 size={12} className="text-[#a3e635] shrink-0" />
                                        {selectedEmpresa.nombre}
                                        {isSubEmpresa && (
                                            <span className="ml-1 px-1.5 py-0.5 rounded bg-[#a3e635]/10 text-[#a3e635] text-[8px] font-black tracking-wider">SUB</span>
                                        )}
                                    </span>
                                )}
                                {formData.categoria_id && (
                                    <>
                                        ...
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-white/10 text-[10px] font-bold text-zinc-300 uppercase tracking-widest">
                                            <Package size={12} className="text-[#a3e635] shrink-0" />
                                            {formData.categoria_id}
                                        </span>
                                    </>
                                )}
                                {selectedProducto && (
                                    <>
                                        <span className="text-zinc-700 text-[10px]">•</span>
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#a3e635]/30 bg-[#a3e635]/5 text-[10px] font-bold text-[#a3e635] uppercase tracking-widest">
                    ...
                    {currentStep === 4 && formData.tipo_solicitud === 'fabricacion' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Categorías en Inventario</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Productos asignados a {empresas.find(e => e.id === formData.empresa_id)?.nombre}</p>
                            </div>
                            ...
                            {categoriasDisponibles.length > 0 ? (
                                    ...
                                    {categoriasDisponibles.map(cat => (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                formData.categoria_id === cat ? "bg-primary/10 border-primary shadow-[0_0_20px_rgba(212,255,112,0.1)]" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                            )}
                                        >
                                            <div className="w-12 h-12 rounded-xl bg-zinc-900 flex items-center justify-center">
                                                <Package className={cn(formData.categoria_id === cat ? "text-primary" : "text-zinc-600")} size={20} />
                                            </div>
                                ...
                                <div className="py-24 text-center">
                                    <div className="flex flex-col items-center gap-4 opacity-40">
                                        ...
                                        <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest max-w-sm mx-auto">Esta empresa aún no tiene productos fabricados o asignados.</p>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {currentStep === 5 && formData.tipo_solicitud === 'fabricacion' && (() => {
                        ...
                        return (
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
                                <div className="text-center space-y-2">
                                    <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Seleccionar Producto</h3>
                                        ...
                                        {filteredProducts.map(item => (
                                            <div
                                                ...
                                                className={cn(
                                                    ...
                                                    formData.producto_id === item.muestrarios_tipos?.id ? "bg-primary/10 border-primary shadow-[0_8px_40px_rgba(212,255,112,0.15)]" : "bg-white/[0.02] border-white/5 hover:border-white/20 hover:scale-[1.02]"
                                                )}
                                            >
                                                <div className="aspect-square rounded-2xl bg-zinc-900 border border-white/5 overflow-hidden relative">
                                                    ...
                                                    {(item.imagen_propuesta_url || item.muestrarios_tipos?.imagen_url) && !brokenLogos.has(item.imagen_propuesta_url || item.muestrarios_tipos?.imagen_url || '') ? (
                                                        ...
                                                        <div className="w-full h-full flex items-center justify-center"><Package className="text-zinc-800" size={40} /></div>
                                                    )}
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-sm font-bold text-white uppercase tracking-tight">{item.muestrarios_tipos?.nombre}</p>
                                                    <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Versión {item.muestrarios_tipos?.version || '1.0'}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    {currentStep === 6 && formData.tipo_solicitud === 'diseno' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Tipo de Muestrario</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Selecciona la base para tu diseño</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                {tiposMuestrario.map(tipo => (
                                    <div
                                        ...
                                        className={cn(
                                            "p-6 rounded-3xl border transition-all duration-300 cursor-pointer text-center flex flex-col items-center gap-4",
                                            formData.producto_id === tipo.id ? "bg-primary/10 border-primary" : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                        )}
                                    >
                                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{tipo.nombre}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {currentStep === 7 && formData.tipo_solicitud === 'diseno' && (
                        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Personalización de Marca</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest">Activos y Estética de la Propuesta</p>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                {/* Logo Group */}
                                <div className="space-y-6">
                                    <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">Gestión de Branding</h4>
                                    <div className="space-y-4">
                                        <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/5 flex items-center gap-4 group hover:border-primary/30 transition-all cursor-pointer">
                                            ...
                                            <div className="flex-1">
                                                <p className="text-xs font-bold text-white">Logo Actual de Empresa</p>
                                                <p className="text-[9px] text-muted-foreground mt-1">Usar archivo predeterminado</p>
                                            </div>
                                            <div className="w-5 h-5 rounded-full border border-white/20 flex items-center justify-center"><Check className="text-primary hidden group-hover:block" size={12} /></div>
                                        </div>
                                        <div className="p-6 rounded-3xl bg-white/[0.02] border border-dashed border-white/10 flex items-center gap-4 hover:border-primary/50 transition-all cursor-pointer">
                                            <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center shrink-0">
                                                <Upload className="text-zinc-600" size={20} />
                                            </div>
                                            <div className="flex-1">
                                                <p className="text-xs font-bold text-white">Subir Nuevo Activo</p>
                                                <p className="text-[9px] text-muted-foreground mt-1">Soporte SVG, PNG (Max 5MB)</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Colors Group */}
                                <div className="space-y-6">
                                    <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">Paleta de Colores (Max 3)</h4>
                                        ...
                                        {['#000000', '#ffffff', '#FF5733', '#33FF57', '#3357FF', '#F333FF', '#33FFF3', '#F3FF33'].map(color => (
                                            <div
                                                ...
                                                className={cn(
                                                    ...
                                                    formData.colores.includes(color) ? "border-primary scale-110 shadow-[0_0_15px_rgba(212,255,112,0.3)]" : "border-white/10"
                                                )}
                    ...
                    {currentStep === 8 && (
                        <div className="max-w-2xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Detalles de Entrega</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Cantidad e Intensidad de Producción</p>
                            </div>

                            <div className="grid gap-8">
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">Cantidad de Unidades</label>
                                    <div className="flex items-center gap-6">
                                        <button
                                            onClick={() => setFormData(prev => ({ ...prev, cantidad: Math.max(1, prev.cantidad - 1) }))}
                                            className="w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-white hover:bg-white/10 active:scale-95 transition-all"
                                        >-</button>
                                        <div className="flex-1 relative">
                                            <input
                                                ...
                                                className="w-full bg-white/5 border border-white/5 rounded-2xl h-14 flex items-center justify-center text-2xl font-black text-white text-center focus:ring-1 focus:ring-primary focus:outline-none transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                            />
                                        </div>
                                        <button
                                    ...
                                    <p className="text-[9px] text-zinc-600 uppercase tracking-widest text-center">Mínimo 1 · Máximo 5000 unidades</p>
                                </div>
                                ...
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        formData.fast_track ? "bg-amber-500/10 border-amber-500/50 shadow-[0_0_30px_rgba(245,158,11,0.1)]" : "bg-white/[0.02] border-white/5 hover:border-amber-500/30"
                                    )}
                                >
                                    <div className="flex items-center gap-6">
                                        <div className={cn(
                                            "w-16 h-16 rounded-3xl flex items-center justify-center transition-all",
                                            formData.fast_track ? "bg-amber-500 text-black shadow-[0_0_20px_rgba(245,158,11,0.4)]" : "bg-zinc-900 text-zinc-600"
                                        )}>
                                            <Zap size={28} fill={formData.fast_track ? "currentColor" : "none"} />
                                        </div>
                                        <div>
                                            <h4 className={cn("text-xl font-bold uppercase tracking-tight", formData.fast_track ? "text-amber-500" : "text-white")}>Fast Track</h4>
                                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">Producción prioritaria · SLA reducido a 2 días</p>
                                        </div>
                                    </div>
                                    <div className={cn(
                                        "w-12 h-6 rounded-full relative transition-all duration-500",
                                        formData.fast_track ? "bg-amber-500" : "bg-white/10"
                                    )}>
                                        <div className={cn(
                    ...
                    {currentStep === 9 && (
                        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Resumen de Solicitud</h3>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest leading-loose">Verifica los detalles antes de enviar</p>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                                <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center"><Building2 className="text-primary" size={20} /></div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Empresa / Marca</p>
                                            <p className="text-sm font-bold text-white uppercase">{empresas.find(e => e.id === formData.empresa_id)?.nombre}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center"><Package className="text-primary" size={20} /></div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Producto / Modelo</p>
                                ...
                                <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center">
                                            {formData.tipo_solicitud === 'fabricacion' ? <Package className="text-primary" size={20} /> : <Palette className="text-violet-400" size={20} />}
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Tipo de Flujo</p>
                                            <p className={cn("text-sm font-bold uppercase", formData.tipo_solicitud === 'fabricacion' ? "text-primary" : "text-violet-400")}>
                                                {formData.tipo_solicitud === 'fabricacion' ? 'Fabricación' : 'Propuesta de Diseño'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center"><Zap className="text-amber-500" size={20} /></div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Prioridad</p>
                                            <p className={cn("text-sm font-bold uppercase", formData.fast_track ? "text-amber-500" : "text-white")}>
                                                {formData.fast_track ? 'Fast Track (Prioritario)' : 'Estándar'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-zinc-900 flex items-center justify-center"><Calendar className="text-primary" size={20} /></div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Fecha Estimada de Entrega</p>
                                            <p className="text-sm font-bold text-white uppercase">
                            ...
                            <div className="pt-8">
                                <textarea
                                    ...
                                    className="w-full bg-white/[0.02] border border-white/5 rounded-[2rem] p-8 text-sm text-white focus:outline-none focus:border-primary/30 transition-all custom-scrollbar h-32"
                ...
                {/* Footer Controls */}
                <div className="p-8 border-t border-white/5 bg-white/[0.01] flex items-center justify-between shrink-0">
                    <button
                        onClick={prevStep}
                        disabled={currentStep === 1}
                        className="px-8 py-4 rounded-2xl border border-white/10 text-zinc-500 hover:text-white hover:bg-white/5 transition-all text-xs font-black uppercase tracking-widest disabled:opacity-0 flex items-center gap-2"
                    >
                        <ChevronLeft size={16} /> Atrás
                    </button>

                    <div className="flex items-center gap-6">
                        <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest hidden md:block">Progreso del Pedido: {Math.round((currentStep / 9) * 100)}%</p>
                        <button
                            onClick={nextStep}
                            className="bg-primary hover:bg-primary/90 text-black font-black px-10 py-4 rounded-2xl flex items-center gap-2 transition-all active:scale-95 shadow-[0_0_20px_rgba(212,255,112,0.15)] text-xs uppercase tracking-widest"
                        >
                ...
                {toast && (
                    <div className={cn(
                        ...
                        toast.type === 'success' ? "bg-primary text-black" : "bg-red-600 text-white"
                    )}>
